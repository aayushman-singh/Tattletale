  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <ChevronDown className="h-4 w-4 text-mute mt-1 flex-shrink-0" />
      </div>
    </GlassCard>
  );
};
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mt-1">
              ...
              <span className="text-pf-google text-sm">Gmail Inbox</span>
            </div>
          </div>
        </div>
...
const EmailCard = ({ email, onSelect }) => {
  return (
    ...
    >
      <div className="flex justify-between items-start">
        <div className="flex-1 min-w-0">
          <h4 className="text-base font-semibold text-pf-google">
            {email.metadata?.subject || "No Subject"}
          </h4>
          ...
          <p className="text-mute text-sm mt-2 line-clamp-2">
            {email.snippet}
          </p>
          {email.attachments && email.attachments.length > 0 && (
            <div className="mt-2">
              <span className="text-xs text-pf-google">
                {email.attachments.length} attachment{email.attachments.length !== 1 ? 's' : ''}
              </span>
            </div>
          )}
        </div>
        <ChevronDown className="h-5 w-5 text-mute mt-1 flex-shrink-0" />
      </div>
    </GlassCard>
  );
};
...
const EmailDetails = ({ email }) => {
  const Attachment = ({ attachment }) => (
    <div className="bg-ink-780/50 p-3 rounded-md text-paper-50 text-sm mt-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-ink-820/40 p-4 rounded-lg mt-4">
        <p className="text-paper-300 whitespace-pre-wrap">{email.snippet}</p>
      </div>

      {email.attachments && email.attachments.length > 0 && (
        <div className="mt-4">
          <h4 className="text-pf-google font-semibold mb-2">
